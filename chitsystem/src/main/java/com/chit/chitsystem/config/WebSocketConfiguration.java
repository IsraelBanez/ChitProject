...
public class WebSocketConfiguration implements WebSocketMessageBrokerConfigurer {
    ...
    // Handles tasks before and after the WebSocket handshake; if successful the client and server can exchange messages.
    // Also, extracts the auth toke from cookie and validates it. If valid, it will be added to the attributes
    @Bean
    ...
    // Extracts the auth token from the attributes and further extracts the userdetails from the token to
    ...
    // communication. Make sure auth token is still valid during communication (not expired or revoked)
    @Override
    public void configureClientInboundChannel(ChannelRegistration registration) {
        registration.interceptors(new ChannelInterceptor() {

            // Intercept messages from the client to server before they are processed further by the server
            @Override
            public Message<?> preSend(Message<?> message, MessageChannel channel) {
                ...
                if (StompCommand.CONNECT.equals(accessor.getCommand())
                    ...
                    if (isTokenVerified) {
                        ...
                        // Or try reconnecting them or check if token expired and if so update the auth token
                    }
                }
                return message;
            }
